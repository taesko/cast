...
from cast import exceptions
from cast.template import registry, core
...
@pytest.mark.parametrize('name, path, error', [
    ...
    (MAIN_TEMPLATE.name + 'not a dir', 'cast/__init__.py', NotADirectoryError),
    (MAIN_TEMPLATE.name + 'not a file', 'not_a_setup.py', FileNotFoundError),
    (MAIN_TEMPLATE.name, 'cast/__init__.py', (NotADirectoryError, exceptions.TemplateExistsError)),
...
@pytest.mark.parametrize('name, instance, error', [
    ...
    (MAIN_TEMPLATE.name, 'cast/__init__.py', NotADirectoryError)
])
